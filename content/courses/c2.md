...
The datset is quite heavy and there are multiple subdataset in the zip, I will use state dataset for our analysis. 
...
### Step 6: Identifying SARIMA Parameter
...
I ran ARIMA/SARIMA model in Both, Stata, R and Python. Unlike STATA both R and Python has an advantage.Fortunately, in python,  we can automate this process using the pmdarima library in Python, which includes the auto_arima function. This function efficiently searches through different combinations of SARIMA parameters and selects the best model based on a specified criterion, typically the AIC (Akaike Information Criterion), which balances model complexity against fit.
...
### Step 9: Forecasting with the SARIMA Model
...
The model's forecast aligns with what might be expected in a healthy real estate market assuming steady growth. However, all models has weakness. **This is no exception.**
